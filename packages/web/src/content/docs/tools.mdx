...
:::note
A Parallel AI API key is required. Set it once in the TUI with `/parallel` (stored in `auth.json`) or export `PARALLEL_API_KEY`. Get a key at [platform.parallel.ai](https://platform.parallel.ai).
:::
...
Performs web searches using [Parallel AI](https://parallel.ai) to find relevant information online. Useful for researching topics, finding current events, or gathering information beyond the training data cutoff.
